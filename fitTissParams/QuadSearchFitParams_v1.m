...
% quadratic is fit to this. The FWHM is used to update limits for next iteration, and the max
...
lowerLimit = [0.03,  15,  8e-6, 1e-4,  15e-3,  0.01]; % set lower limits for fit
highLimit  = [0.12,  80, 14e-6, 25e-3, 110e-3,  1.5]; % set lower limits for fit
...
HL = zeros(maxIterations*6+1,6); HL(1,:) = highLimit;
LL = zeros(maxIterations*6+1,6); LL(1,:) = lowerLimit;
...
while iteration <= maxIterations
    ...
    for i = 1:num2fit % for each variable:
        ...
        if limitPerDif > 0.05
            ...
            fitMetricY = fitPolyCompute4StdResid(sigL, sigM, sigH, sigMH,...
                fitData, b1Field);
            ...
            % Update limits:
            [lowerLimit,highLimit, vertex] = updateFitLimitsQuad(vertex, lowerLimit,...
...
i = 3;
x =[ lowerLimit(i);currList(i);highLimit(i); fp];
% x1 = x1*10^6;
x2 = log10(x);
...
t1d = [1e-4 3e-4 7e-4 1e-3 5e-3 1e-2 2e-2];
t1d2 = log10(t1d);
t1d2 = 10.^(t1d2);
...
x = [lowerLimit(i);currList(i);highLimit(i); ( currList(i)+highLimit(i))/2];
                fitMetric
