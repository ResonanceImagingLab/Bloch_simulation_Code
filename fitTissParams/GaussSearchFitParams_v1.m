%% Gaussian search fit params

% start with an initial guess of fit parameters. 
% For each iteration, we compute signal curves (over B1+) for 3 tissue
% paramter options. The fit is evaluated as the inner product, and then a
% gaussian is fit to this with the limits constrained to be within the set
% limits, with the amplitude set equal to the inner product of the data 
% with itself. The FWHM is used to update limits for next iteration, and the max
% is set to the new best guess. This  is repeated on each variable (one at a
...
currList   = [0.07,  35, 11e-6, 1e-3,    50e-3,  1]; % current estimate/start
lowerLimit = [0.02,  15,  7e-6, 0.05e-3, 10e-3,  0.01]; % set lower limits for fit
highLimit  = [0.12,  90, 14e-6, 40e-3,  120e-3,  1.5]; % set lower limits for fit
...
b1L = min(fitData(10,:))*0.95; % go just past for better fitting in area of interest
b1H = max(fitData(10,:))*1.05;
...
maxDot = 0; % max value for fit reference
for i = 1:6
    maxDot = maxDot + dot(1+fitData(i,:), 1+fitData(i,:));
end


trackingMat = zeros(10000,8); % store each variable, and the calculate FWHM
...
while iteration < maxIterations
    ...
    for i = 1:num2fit % for each variable:
        ...
        fitMetricY = fitPolyComputeInnerProd(sigL, sigM, sigH, fitData, b1Field);

        % From this, we want to sum across the sequences to combine the
        % scores. highest wins
        fitMetricY = sum(fitMetricY,2);

        % Use these 3 data points to fit a gaussian to the fit results to
        % try and estimate where the peak will be (fit parameter!). Use
        % FWHM to update low and high limits so we hopefully narrow over
        % time. Do not allow them to exceed current limits

        cVal = createGaussFit([lowerLimit(i);currList(i);highLimit(i)],...
            fitMetricY, maxDot);
        
        currList(i) = cVal(2); % new guess
        disp(['Current parameters are estimated as: ', num2str(currList)]);

        % Update limits:
        [lowerLimit,highLimit] = updateFitLimits(cVal, lowerLimit,highLimit, i);
