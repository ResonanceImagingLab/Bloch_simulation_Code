function fitMetric = fitPolyCompute4StdResid2raw(sigL, sigM, sigH, sigMH,...
                                        rawproc, b1Field)

% sigL, sigM, sigMH and sigH hold sat values, need to fit each with a 3th degree
% polynomial. Then evaluate that polynomial using the acquired data points.
% b1Field is the B1 values corresponding to sigL...
% the In this weighted version, the fit is weighted by the b1 value
...
for i = 1:6     
     ...
     fitMetric(1,i) = CR_calc_std_residualsWeighted( rawproc(10,:), rawproc(i,:) , fitCoeffMat);

     fitCoeffMat = polyfit(b1Field, sigM(:,i), fit_degree);
     fitMetric(2,i) = CR_calc_std_residualsWeighted( rawproc(10,:), rawproc(i,:) , fitCoeffMat);

     fitCoeffMat = polyfit(b1Field, sigH(:,i), fit_degree);
     fitMetric(3,i) = CR_calc_std_residualsWeighted( rawproc(10,:), rawproc(i,:) , fitCoeffMat);

     fitCoeffMat = polyfit(b1Field, sigMH(:,i), fit_degree);
     fitMetric(4,i) = CR_calc_std_residualsWeighted( rawproc(10,:), rawproc(i,:) , fitCoeffMat);
end
