...
Params2.WExcDur = 3/1000; % duration of water pulse
...
Params3.WExcDur = 3/1000; % duration of water pulse
...
parametersSet = zeros( simLength,  6);
...
save(strcat(SavDir,'parametersSet.mat'),'parametersSet') % save this just incase I change it at some point...




%% Run simulations
Single_sig1 = zeros( simLength, Params.TurboFactor, length(B1rms));
Single_sig2 = zeros( simLength, Params2.TurboFactor, length(B1rms));
Single_sig3 = zeros( simLength, Params3.TurboFactor, length(B1rms));
Dual_sig1 = zeros( simLength, Params.TurboFactor,  length(B1rms));
Dual_sig2 = zeros( simLength, Params2.TurboFactor, length(B1rms));
Dual_sig3 = zeros( simLength, Params3.TurboFactor, length(B1rms));
...
Single_sig1_gm = zeros( simLength, length(B1rms));
Single_sig2_gm = zeros( simLength, length(B1rms));
Single_sig3_gm = zeros( simLength, length(B1rms));
Dual_sig1_gm = zeros( simLength,  length(B1rms));
Dual_sig2_gm = zeros( simLength, length(B1rms));
Dual_sig3_gm = zeros( simLength,  length(B1rms));
...
Top50sorted2 = parametersSet ( sortidx2(1:end),:);
...
TParams  = fitTissueParamsFromLUT( parametersSet, sum(abs(standardized_residuals),2) , 20);
TParams2 = fitTissueParamsFromLUT( parametersSet, sum(abs(standardized_residuals2),2) , 20);

 
 
 %% take the average of the tissue parameters between two subjects
T_avg = (TParams + TParams2)./2;
...
Single_sig1_gm_f = zeros( simLength, length(B1rms));
Single_sig2_gm_f = zeros( simLength, length(B1rms));
Single_sig3_gm_f = zeros( simLength, length(B1rms));
Dual_sig1_gm_f = zeros( simLength,  length(B1rms));
Dual_sig2_gm_f = zeros( simLength, length(B1rms));
Dual_sig3_gm_f = zeros( simLength,  length(B1rms));
...
% Make sure you have B1rms values! Multiply B1 map by the B1rms of the sequence 
b1_1_1 =11.4* mat_ss(10,:) ; 
b1_2_1 = 13.3 * mat_ss(10,:) ; 
b1_3_1 = 11.6*mat_ss(10,:) ; 
...
str = ['R = ',num2str(T_avg(1)),', T2a = ',num2str(T_avg(2)),...
    ', T1D = ',num2str(T_avg(3)), ', T2b =',num2str(T_avg(4)),...
    ', M0b = ',num2str(T_avg(5)),', R1b = ',num2str(T_avg(6))];
...
hold off                     
 ...
 sgtitle(str)          
   ...
   saveas(gcf,strcat(SavDir,'Final_best_parameters_fit2Optimal.png'))  
