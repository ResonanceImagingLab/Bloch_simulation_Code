function LUT = compute_2pt_inversion_LUT(T1wInfo, PDwInfo, T1w_b, T1w_fftb, PDw_b, PDw_fftb, LUT_str, LUTsavDir)
...
T1wInfo.M0b = 0; % set to 0 to ignore MT effects
PDwInfo.M0b = 0; % set to 0 to ignore MT effects
...
for i = 1:length(B1_vector)           
    parfor  m = 1:length(T1_vector)
        ...
        FLASH_vector = BlochSimFlashSequence_v2(PDwInfo, 'flipAngle',...
                B1_vector(i)*PDwInfo.flipAngle,...
                'Raobs', 1./T1_vector(m)*1000 );

        FLASH_vector = FLASH_vector*exp(-PDwInfo.TE *PDwInfo.R2star);
        ...
        MPRAGE_vector = BlochSim_MPRAGESequence(T1wInfo, 'flipAngle',...
        ...
        MPRAGE_v = CR_generate_BSF_scaling_v1( MPRAGE_vector, T1wInfo, T1wSamplingTable, T1w_b, T1w_fftb); 
