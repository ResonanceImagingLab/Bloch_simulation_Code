function Params = CalcBoundSatFrom2ExcitationPulse(Params, flipAngle1, flipAngle2)
...
b1 = flipAngle1 /(360*42.577478518*Params.WExcDur);
...
if strcmp(Params.lineshape, 'SuperLorentzian') % default is gaussian
    Params.Rrfb_exc1 = pi.*w1.^2.* G;    
elseif strcmp(Params.lineshape, 'Lorentzian') % default is gaussian
    Params.Rrfb_exc1 = pi.*w1.^2.* G;    
else
    Params.Rrfb_exc1 = w1.^2 .* G;
end

% For dipolar pool == 0 since delta = 0.
Params.Rrfd_exc = 0; % Params.Rrfb_exc*(2*pi*delta/wloc)^2;

%% For second flip angle:
b1 = flipAngle2 /(360*42.577478518*Params.WExcDur);
w1 = 2*pi *42.577478518 * b1;  % assume B1 is in microTesla, and drop the 10^6 from gamma. w1 in rad/s

if strcmp(Params.lineshape, 'SuperLorentzian') % default is gaussian
    Params.Rrfb_exc2 = pi.*w1.^2.* G;    
elseif strcmp(Params.lineshape, 'Lorentzian') % default is gaussian
    Params.Rrfb_exc2 = pi.*w1.^2.* G;    
else
    Params.Rrfb_exc2 = w1.^2 .* G;
end
