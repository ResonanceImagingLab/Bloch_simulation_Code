function [outSig, M, time_vect] = BlochSim_MPRAGESequence_1pool(Params, varargin)
...
% Only Consider a single pool (no exchange).
...
% seems like the scanner takes the time to be from the end of the 180 for
% Siemens
if strcmp(Params.Readout, 'centric')
    ET = TI ; 
else % assume linear encoding
...
for i = 1:loops
    ...
    R = RotationMatrix_1pool_Inversion( pi*Params.InversionEfficiency, 0);
    ...
    M_t = XYmag_Spoil_1pool(Params, M_t, ET, 0, 0);
