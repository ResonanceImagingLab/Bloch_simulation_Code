function CR_Siemens_generate_R1_Volumes_v2(location, B1scale)
...
LUTsavDir = '/home/bockn/matlab/Longitudinal_ICM_in_BD/BD_SeqSimCode/LUT_files/'; 
...
sub_fldr =  SUBJECTS(1);   
filenameL = strcat(BaseFldr, sub_fldr, '/MNI_152/', CONTRASTS(3),'.nii' );
...
%% B1
        ...
        if ~isfile(filenameL) % File exists.
            missingData{idx} = strcat(BaseFldr, sub_fldr, '/MNI_152/', CONTRASTS(3),'.nii' );
            idx = idx+1; 
            continue;
        end
...
%% T1w
        filenameL = strcat(BaseFldr, sub_fldr, '/MNI_152/', CONTRASTS(1),'.nii' );
        
        if ~isfile(filenameL) % File exists.
            missingData{idx} = strcat(BaseFldr, sub_fldr, '/MNI_152/', CONTRASTS(1),'.nii' );
            idx = idx+1; 
            continue;
        end
...
%% PDw
        filenameL = strcat(BaseFldr, sub_fldr, '/MNI_152/', CONTRASTS(2),'.nii' );
        
        if ~isfile(filenameL) % File exists.
            missingData{idx} = strcat(BaseFldr, sub_fldr, '/MNI_152/', CONTRASTS(2),'.nii' );
            idx = idx+1; 
            continue;
        end
...
%% calculate R1
        ...
        T1 = Siemens_2pt_Inversion_T1mapping(T1wInfo, PDwInfo, T1W,...
                PD, B1, [], LUTsavDir, [], [] );
...
%% Put back into cube
        ...
        % output Name:
        if NORMALIZED==1
           outputName = strcat(OutFldr, sub_fldr, '/MNI_152/R1_map_normalized.nii' );  
        % Clean up
        temp=strcat(BaseFldr, sub_fldr, '/MNI_152/R1_map_normalized.nii.gz');
        delete temp
        Hdrinfo.Datatype = 'double'; % change from single
        niftiwrite(R1_vol, char(outputName), Hdrinfo, 'Compressed',false);

        else
        mkdir (strcat(OutFldr, sub_fldr, '/MNI_152/'))
        outputName = strcat(OutFldr, sub_fldr, '/MNI_152/R1_map.nii' );
        
        % Clean up
        temp=strcat(BaseFldr, sub_fldr, '/MNI_152/R1_map.nii.gz');
        delete temp
        Hdrinfo.Datatype = 'double'; % change from single
        niftiwrite(R1_vol, char(outputName), Hdrinfo, 'Compressed',false);
        end
