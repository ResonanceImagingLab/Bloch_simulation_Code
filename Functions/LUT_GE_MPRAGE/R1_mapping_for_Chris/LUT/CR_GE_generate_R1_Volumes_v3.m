function CR_GE_generate_R1_Volumes_v3(location, B1scale, ratioscale)

addpath(genpath('/home/bockn/matlab/R1_Mapping_Longitudinal_ICM_in_BD'))
...
headerDir = '/home/bockn/matlab/R1_Mapping_Longitudinal_ICM_in_BD/';
...
elseif location == 6
    ...
    error('Location must be a value between 1 and 6')
% elseif location == 4
%     SITE = 'Queens';
% elseif location == 5
%     SITE = 'UNH';
...
for j=1:2
    ...
    % Fix for folder naming problem
    if B1scale==0;
    OutFldr = strcat(DATA_DIR, SITE, '/R1_maps_0_',num2str(ratioscale),'/');
    else
    OutFldr = strcat(DATA_DIR, SITE, '/R1_maps_',num2str(B1scale),'_',num2str(ratioscale),'/');
    end
    ...
    sub_fldr =  SUBJECTS(contains(SUBJECTS,ExamNames{2}));   
...
% Breaks if there was no missingData'
