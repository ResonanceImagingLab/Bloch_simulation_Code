...
function T1map = GE_2pt_Inversion_T1mapping(T1wInfo, PDwInfo, MPRAGE, PDw, B1, mask, LUTsavDir, lutPrefix)
%function T1map = GE_2pt_Inversion_T1mapping(T1wInfo, PDwInfo, MPRAGE, PDw, B1, mask, LUTsavDir, MNIdir, MNISavDir )
...
checkGEparamObject(T1wInfo, PDwInfo );
...
%       PDw_fftb = load(strcat(MNISavDir,str2,'/','fft_brain_m.mat'));
        ...
        LUT = compute_2pt_inversion_LUT_1pool(T1wInfo, PDwInfo, LUT_str, LUTsavDir, T1w_b, T1w_fftb);  
    else
