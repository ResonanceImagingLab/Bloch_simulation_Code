...
outDir = 'E:\Github\Bloch_simulation_Code\Investigations\Neuromelanin\batchOutput';
...
Offset_freq = 3000; 
flipAngle = 4:2:16;
pulseDur = [3, 5, 7, 10]./1000; % in seconds
TR = [70, 100, 150, 200, 250, 300, 400, 500,650, 800, 1000 ]./1000; % in seconds
numSatPulse = 4:2:10;
numExc = [ 10, 16, 24, 32, 48, 64, 78, 90];
Params.boosted = 1; % 
...
plot(cnr2_idx);
plot(cnr3_idx); legend

%% jumpy but consistent trends. Lets take root mean square to weight to the best combined CNR
...
%% Flip angle is pretty high... lets look at Flip angle = 8; to match privoulous
...
%% Flip angle is pretty high... lets look at Flip angle = 6; to match privoulous
temp = [ParameterSet(cnr4_idx,:), satFlipAngle_val(cnr4_idx,:), ...
    cnr_bs_sn(cnr4_idx), cnr_bs_lc(cnr4_idx), cnr_csf_lc(cnr4_idx)  ];
temp(temp(:,2) > 6, :)= [];
